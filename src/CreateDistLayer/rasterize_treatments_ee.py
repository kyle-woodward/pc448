...
logging.basicConfig(
    ...
    filename=os.path.join(os.path.dirname(__file__),'rasterize_treatments_ee.log')
)
...
def main():
    ...
    dist_w_ranks = ee.FeatureCollection(f"projects/pyregence-ee/assets/treatments/dist_w_ranks_{version}")
    
    ranksImg = dist_w_ranks.reduceToImage(['ranks'], ee.Reducer.max()).rename('ranks')

    logger.info(ranksImg.bandNames().getInfo())

    desc = f'dist_treatmentsRanks_EERasterizeDistWRanks_{version}'
    asset_id = f"projects/pyregence-ee/assets/workflow_assets/{desc}"        
